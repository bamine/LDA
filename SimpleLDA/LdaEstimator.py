...
class LdaEstimator(object):
    ...
    def save_gamma(self,fileName,gamma,num_docs,num_topics):
        f=open(fileName)
    ...
    def run_EM(self,startFileName,directory,corpus,num_topics,startType=None):

        em_converged=10
        max_iter=10
        var_gamma=np.zeros(corpus.num_docs)
        phi=np.zeros(num_topics,max(corpus.docs,key=lambda d: d.length))
        if startType=="seeded":
        ...
        likelihood_file=open(likelihood_filename)
        i=0
        while(converged <0 or converged > em_converged or (i<=2 and i<=max_iter)):
            ...
            for d in xrange(corpus.num_docs):
                if d%1000==0:
                    print "document {0}"%d
                    likelihood+=self.docEStep(corpus.docs[d],var_gamma[d],phi,model)
            ...
            if i%10==0:
                ...
                self.save_gamma(my_file,var_gamma,corpus.n_docs,model.num_topics)
